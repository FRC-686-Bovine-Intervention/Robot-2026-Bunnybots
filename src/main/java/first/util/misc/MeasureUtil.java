...
import first.util.math.MathExtraUtil;
...
public class MeasureUtil {
	...
	public static <U extends Unit> Measure<U> clamp(Measure<U> value, Measure<U> low, Measure<U> high) {
		return (Measure<U>)value.unit().ofBaseUnits(MathExtraUtil.clamp(value.baseUnitMagnitude(), low.baseUnitMagnitude(), high.baseUnitMagnitude()));
	}

	@SuppressWarnings("unchecked")
	public static <U extends Unit> Measure<U> interpolate(Measure<U> start, Measure<U> end, double t) {
		return (Measure<U>)start.unit().ofBaseUnits(MathUtil.lerp(start.baseUnitMagnitude(), end.baseUnitMagnitude(), t));
	}
	public static <U extends Unit> double inverseInterpolate(Measure<U> start, Measure<U> end, Measure<U> t) {
		return MathUtil.inverseLerp(start.baseUnitMagnitude(), end.baseUnitMagnitude(), t.baseUnitMagnitude());
	}
}
